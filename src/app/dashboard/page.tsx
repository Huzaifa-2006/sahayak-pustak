...
import { Star, BookOpen, FileText, Trophy, Plus, BookMarked, Upload } from "lucide-react";
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="card overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
              ...
              <Link href="/upload/book" className="text-xs font-medium text-brand-600 hover:underline">
                + Add New
              </Link>
            </div>
            {userBooks.length === 0 ? (
              ...
              <div className="divide-y divide-slate-50">
                {userBooks.map((book) => (
                  <Link key={book.id} href={`/books/${book.id}`} className="flex items-center gap-4 px-6 py-3.5 hover:bg-slate-50 transition-colors">
                    ...
                    <div className="text-right shrink-0">
                      ...
                      {book.isDonation && (
                        <span className="text-[10px] font-semibold text-green-600">DONATED</span>
                      )}
                    </div>
                  </Link>
                ))}
          ...
          <div className="card overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
              ...
              <Link href="/upload/note" className="text-xs font-medium text-brand-600 hover:underline">
                + Upload
              </Link>
            </div>
            {userNotes.length === 0 ? (
              ...
              <div className="divide-y divide-slate-50">
                {userNotes.map((note) => (
                  <div key={note.id} className="flex items-center gap-4 px-6 py-3.5">
                    ...
                    <div className="text-right shrink-0">
                      <p className="text-xs text-slate-400">{note.downloadCount} downloads</p>
                    </div>
                  </div>
                ))}
              </div>
